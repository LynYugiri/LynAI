...
class LuaStateImpl implements LuaState, LuaVM {
  ...
  @override
  void pushAsyncDartFunction(AsyncDartFunction f) {
    _stack!.push(Closure.DartFunc(f, 0, canYield: true));
  }
  ...
  Future<ThreadStatus> pCallAsync(
    ...
    try {
      call(nArgs, nResults);
      return ThreadStatus.luaOk;
    } on LuaYieldRequest catch (yielded) {
      try {
        var request = yielded.request;
        while (true) {
          if (_yieldedCalls.isEmpty) {
            throw Exception('yield outside resumable CALL');
          }
          final callSite = _yieldedCalls.removeAt(0);
          final resumeValue = await handler(request);
          _pushDartValue(resumeValue);
          Instructions.popResults(callSite.a, callSite.c, this);
          try {
            while (true) {
              _runLuaClosure();
              if (_stack == caller || _yieldedCalls.isEmpty) break;
              final parentCall = _yieldedCalls.removeAt(0);
              _returnCurrentLuaFrame(parentCall.c - 1);
              Instructions.popResults(parentCall.a, parentCall.c, this);
            }
            if (_yieldedCalls.isEmpty) break;
          } on LuaYieldRequest catch (next) {
            request = next.request;
          }
        }
        final finished = _stack!;
        final nRegs = finished.closure!.proto!.maxStackSize;
        final results = finished.popN(finished.top() - nRegs);
        _popLuaStack();
  ...
  @override
  Never yieldAsync(Object? request) {
    throw LuaYieldRequest(request);
  }

  @override
  void pushCoroutineFrom(int idx) {
    final value = _stack!.get(idx);
    if (value is! Closure || value.proto == null) {
      throw Exception('function expected');
    }
    final userdata = newUserdata<LuaCoroutine>();
    final coroutine = LuaCoroutine(value);
    coroutine.handle = userdata;
    userdata.data = coroutine;
  }

  @override
  int resumeCoroutine(int idx, int nArgs) {
    final coroutine = toUserdata<LuaCoroutine>(idx)?.data;
    if (coroutine == null) {
      pushBoolean(false);
      pushString('thread expected');
      return 2;
    }
    if (coroutine.status == LuaCoroutineStatus.dead) {
      pushBoolean(false);
      pushString('cannot resume dead coroutine');
      return 2;
    }
    if (coroutine.status == LuaCoroutineStatus.running) {
      pushBoolean(false);
      pushString('cannot resume running coroutine');
      return 2;
    }
    final caller = _stack!;
    final args = nArgs > 0 ? caller.popN(nArgs) : const <Object?>[];
    caller.pop();
    try {
      coroutine.status = LuaCoroutineStatus.running;
      _runningCoroutine = coroutine;
      if (!coroutine.started) {
        coroutine.started = true;
        _startCoroutine(coroutine, args);
      } else {
        _resumeCoroutineStack(coroutine, caller, args);
      }
      _runLuaClosure();
      final results = _collectCurrentCoroutineResults();
      coroutine.status = LuaCoroutineStatus.dead;
      coroutine.yieldedCalls.clear();
      _runningCoroutine = null;
      _stack = caller;
      pushBoolean(true);
      _stack!.pushN(results, -1);
      return results.length + 1;
    } on LuaYieldRequest catch (yielded) {
      final values = yielded.request is List
          ? List<Object?>.from(yielded.request as List)
          : <Object?>[yielded.request];
      coroutine.status = LuaCoroutineStatus.suspended;
      coroutine.yieldedCalls
        ..clear()
        ..addAll(_yieldedCalls);
      _yieldedCalls.clear();
      coroutine.stack = _detachCoroutineStack(caller);
      _runningCoroutine = null;
      _stack = caller;
      pushBoolean(true);
  ...
  @override
  Never yieldCoroutine(int nResults) {
    if (_runningCoroutine == null) {
      throw Exception('attempt to yield from outside a coroutine');
    }
    final values = nResults > 0 ? _stack!.popN(nResults) : const <Object?>[];
    throw LuaYieldRequest(values);
  }

  @override
  String coroutineStatus(int idx) {
    final LuaCoroutine? coroutine = toUserdata<LuaCoroutine>(idx)?.data;
    if (coroutine == null) throw Exception('thread expected');
    return switch (coroutine.status) {
      LuaCoroutineStatus.suspended => 'suspended',
      LuaCoroutineStatus.running => 'running',
      LuaCoroutineStatus.dead => 'dead',
    };
  }

  @override
  int pushRunningCoroutine() {
    final coroutine = _runningCoroutine;
    if (coroutine?.handle == null) {
      pushNil();
      pushBoolean(true);
      return 2;
    }
    _stack!.push(coroutine!.handle);
    pushBoolean(false);
    return 2;
  }
  ...
  @override
  void markYieldedCall(Object? request, int a, int c) {
    _yieldedCalls.add(LuaYieldedCallSite(a, c));
  }

  void _startCoroutine(LuaCoroutine coroutine, List<Object?> args) {
    final closure = coroutine.closure;
    final nRegs = closure.proto!.maxStackSize;
    final nParams = closure.proto!.numParams!;
    final isVararg = closure.proto!.isVararg == 1;
    final stack = LuaStack();
    stack.state = this;
    stack.closure = closure;
    stack.pushN(args, nParams);
    if (args.length > nParams && isVararg) {
      stack.varargs = args.sublist(nParams);
    }
    _pushLuaStack(stack);
    setTop(nRegs);
  }

  void _resumeCoroutineStack(
    LuaCoroutine coroutine,
    LuaStack caller,
    List<Object?> args,
  ) {
    final stack = coroutine.stack;
    if (stack == null) throw Exception('coroutine stack missing');
    _yieldedCalls
      ..clear()
      ..addAll(coroutine.yieldedCalls);
    coroutine.yieldedCalls.clear();
    _attachCoroutineStack(stack, caller);
    _stack = stack;
    if (_yieldedCalls.isNotEmpty) {
      final callSite = _yieldedCalls.removeAt(0);
      _stack!.pushN(args, -1);
      Instructions.popResults(callSite.a, callSite.c, this);
    }
  ...
  List<Object?> _collectCurrentCoroutineResults() {
    final finished = _stack!;
    final nRegs = finished.closure!.proto!.maxStackSize;
    final results = finished.popN(finished.top() - nRegs);
    _popLuaStack();
  ...
  LuaStack _detachCoroutineStack(LuaStack caller) {
    final top = _stack!;
    var cursor = top;
    while (cursor.prev != null && cursor.prev != caller) {
      cursor = cursor.prev!;
    }
    if (cursor.prev == caller) cursor.prev = null;
    return top;
  }

  void _attachCoroutineStack(LuaStack top, LuaStack caller) {
    var cursor = top;
  ...
  void _returnCurrentLuaFrame(int nResults) {
    final finished = _stack!;
    final nRegs = finished.closure!.proto!.maxStackSize;
    final results = finished.popN(finished.top() - nRegs);
    _popLuaStack();
  ...
  void _pushDartValue(Object? value) {
    if (value == null) {
      pushNil();
      ...
      createTable(value.length, 0);
      final table = absIndex(-1);
      for (var i = 0; i < value.length; i++) {
        _pushDartValue(value[i]);
        setI(table, i + 1);
      }
    } else if (value is Map) {
      createTable(0, value.length);
      final table = absIndex(-1);
      for (final entry in value.entries) {
        _pushDartValue(entry.key);
        _pushDartValue(entry.value);
        setTable(table);
      }
    } else {
