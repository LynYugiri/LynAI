...
LynAI 是一个本地优先的跨平台 AI 客户端。它把多模型聊天、附件理解、Markdown/LaTeX 渲染、本地工具、笔记、日程、待办、情景演绎和备份恢复放在同一个 Flutter 应用里。
...
| 数据迁移 | ZIP 备份、分区导入、冲突处理、附件恢复和 storage_v2 迁移。 |
...
| 工具调用 | 模型可通过受控工具读取或修改本地日程、笔记和待办。 |
| 功能页 | Dashboard、对话历史、日程、笔记、待办、情景演绎。 |
| 角色与提示词 | 聊天角色、角色分组、系统提示词模板、角色默认模型和主题色。 |
| 外观 | Material 3、主题色、浅色/深色/跟随系统、背景图和毛玻璃。 |
| 数据管理 | 备份导出、备份预览、分区导入、storage_v2 迁移和私有资源恢复。 |
...
| 平台 | 说明 |
|------|------|
| Android | 支持定位、打开应用、保存图库、日程小组件、通知和前台生成服务。 |
...
LynAI 通过 Repository 层选择本地存储。storage_v2 激活后，结构化数据写入 Drift 数据库，笔记分页正文写入 Markdown 文件，资源文件写入应用私有目录；SharedPreferences JSON 作为旧数据来源和兼容路径保留。
...
附件不会嵌入对话 JSON。页面会先复制到应用私有目录，再把路径和元数据保存到模型对象中。旧 Documents 路径会在启动加载后迁移到当前私有资源目录。

## 项目结构

```text
lib/
├── main.dart                  # Provider 注册、启动加载、迁移、主题构建
├── models/                    # 可序列化数据契约
├── providers/                 # ChangeNotifier 状态和保存队列
├── repositories/              # storage_v2 / legacy SharedPreferences 持久化
├── services/                  # API、工具、备份、迁移、平台能力
├── pages/                     # 主界面、设置页、功能页和子页面
├── utils/                     # 文件名、分享、SnackBar、更新日志解析等工具
└── widgets/                   # Markdown/LaTeX 和通用 UI 组件
```
...
| [服务层](doc/services.md) | API、工具调用、备份、存储迁移和平台能力。 |
