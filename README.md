...
| 模型目录 | 可选地从公开的 models.dev 目录补全上下文窗口、输出上限、视觉/工具/思考能力与思考强度取值（只填空缺，不覆盖手填值）；数据来自后端 `/models/catalog` 代理、直连 models.dev 或包内离线快照，缓存只存公开目录，不上传 API Key 或对话内容。 |
...
一个 Chat Provider 可以包含多个子模型。只有启用的子模型会出现在选择器里；子模型的能力和采样参数优先于 Provider 级设置。子模型的上下文窗口按「本机覆盖 > 手填 > Endpoint 拉取 > 模型目录建议 > Provider 级 > 默认 256k」生效；`设置 -> 模型目录` 可以查看目录状态、立即刷新、为已配置模型批量补全参数，或在模型编辑页按需补全。思考强度在对话输入栏的思考开关右侧选择（模型目录给出取值时出现），模型编辑页可以设置该模型的默认强度。
...
Agent 对话可通过 `create_plugin` 从零生成插件草稿，成功后对话绑定该插件为工作区并在消息流中展示草稿卡；插件工坊顶部「交给 AI 修改」可打开绑定该插件的对话继续迭代，返回工坊后自动刷新文件树和恢复点。
...
社区帖子、评论、互动和图片属于所配置后端的远程公开数据域，不写入本地 `storage_v2`，也不进入应用备份、云同步或局域网同步。切换后端或账号时，社区页面会清除旧作用域的远程状态并重新加载。
