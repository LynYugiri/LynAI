...
- **Markdown 与 LaTeX**：消息和笔记支持 Markdown、代码块、内联公式 `$...$`、块级公式 `$$...$$`、`\(...\)`、`\[...\]`。
...
- **本地工具调用**：模型可调用本地工具获取时间、位置、打开 Android 应用、管理日程和笔记；不支持原生 tool_calls 的接口可走 JSON fallback。
...
| 日程表 | 支持月视图、周时间轴、全年总览，可新增、编辑、删除日程和备注 |
