...
Agent Lua 可以通过 `lynai.call()` 调用这些函数，也可以用 `lynai.model.generateImage(args)` 发起异步生图命令。Lua 源码不做固定长度截断，`lynai.call` 不做固定次数硬限制；设备任务依赖悬浮层和 `DeviceRunController` 的暂停/停止机制中断。手机复杂操作优先使用 `device.screen.query` 或 `device.node.findAll` 查找任务相关节点；确实需要完整结构时再读取 `device.screen.snapshot`。自绘或无障碍信息不足时可调用 `device.screen.screenshot`，但截图 base64 只能作为 `model.ocr` 或 `model.recognizeFile` 的输入，回传模型的 tool result 会剥离二进制字段，只保留 OCR/识图文本和截图元数据。
...
7. Agent 模式提供 `run_subagent`，用于把手机自动化、屏幕读取和 OCR/识图等高噪声子任务隔离到独立上下文，主对话只接收最终结构化结果。
...
| `lynai/device_control` | snapshot/context/query/screenshot/tap/swipe/inputText/nodeAction | Android | 通过无障碍读取屏幕、筛选节点、截屏并执行手机操作。 |

读屏类函数 `device.screen.*`、`device.node.find`、`device.node.findAll` 和 `device.waitForNode` 需要 `device:screen:read`。动作类函数如 `device.app.open`、`device.tap`、`device.swipe`、`device.inputText` 和 `device.node.action` 需要 `device:control`。

### Agent Subagent

`run_subagent` 是 Agent 专用工具。它使用当前对话模型和权限创建独立短上下文，允许子任务多轮调用 `execute_lua`、Skill、OCR/识图等工具，但不会把中间屏幕信息写入主对话上下文。Subagent 禁止递归启动 Subagent，不设置固定工具轮数上限，最终必须返回 `{ok:true,result:{...}}` 或 `{ok:false,error:{...}}`。
