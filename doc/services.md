...
Agent Lua 可以通过 `lynai.call()` 调用这些函数，也可以用 `lynai.model.generateImage(args)` 发起异步生图命令。手机复杂操作通常先读取 `device.screen.context`，自绘或无障碍信息不足时再调用 `device.screen.screenshot`，并把截图传给 `model.ocr` 或 `model.recognizeFile`。
