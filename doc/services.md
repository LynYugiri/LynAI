...
目录只给 `budget_tokens` 的预算型模型（例如直连 Anthropic 的 Claude）没有厂商 effort 取值，客户端用通用档位 `low`/`medium`/`high` 表达，并按 `reasoningEffortBudgets` 换算成 `thinking.budget_tokens`（显式 `thinkingBudgetTokens` 仍然优先，结果夹在 `max_tokens-1` 内）。托管 relay 上同样三个档位会以 `reasoning.effort` 下发，由后端按上游格式换算。
