...
| `lynai/floating_assistant` | showBubble/hideBubble/hideForScreenshot/restoreAfterScreenshot/configure/updateChatState/updateAgentPlan/setTranslationRunning/updateTranslationOverlay/clearTranslationOverlay | Android | 系统级悬浮聊天窗，接收 Dart 聊天状态、发送用户输入、语音填充输入框、触发翻译（toggle 可中途停止）和 Agent 控制。支持气泡/面板拖动（修正垂直边界 clamp）、面板缩放（防越屏）、位置持久化（拖动去抖）、气泡状态指示（脉冲动画）、消息长按复制、译文卡菜单复制/清空、新建对话。 |
...
气泡可拖动（垂直边界限制）并记忆位置；面板 header 可拖动、底部可缩放（受面板 x 限制防越屏），位置和尺寸持久化到 `FloatingAssistantSettings`。气泡根据状态变色并脉冲动画：蓝色空闲、橙色 Agent 运行、绿色翻译中、红色录音中。面板消息长按可复制到剪贴板；译文卡单击弹出"复制/清空"菜单，长按直接复制。

翻译流程 OCR 优先（截图 → `device.screen.ocr` → PPOCRv5），无障碍节点树为兜底。截图与 OCR 之前会先调用 `hideForScreenshot` 临时移除气泡、面板和上一屏翻译覆盖层并等帧一次，避免悬浮窗自身出现在 framebuffer 中被 OCR 回灌（自翻译反馈环）；完成后 `restoreAfterScreenshot` 原位恢复。块 ID 按 `originalText|l,t,r,b`（每维 ~8px 容差）稳定哈希生成，避免 native 位置序列号 `ocr_$i` 跨屏碰撞导致 cache 误命中、误把旧译文贴到新块上。可屏蔽指定包名应用（`blockedPackages`）、指定专用翻译模型（`translationModelId`，缺省时跟随当前聊天模型）。`mangaLayoutMode` 横排/竖排会真正改变译文渲染：竖排单行 `ellipsize=END`，横排高度 `WRAP_CONTENT` 让长译文向下扩展而非裁剪；auto 模式优先采用 OCR 给出的 `orientation` 标志判定竖排，仅在缺省时才回退到 AABB 的 `h > w` 经验值，避免放大后 AABB 抖动导致的同块跨帧模式切换。`FloatingAssistantOverlay` 在 Activity `onDestroy` 时调用 `uninstall()` 释放广播接收器和 Activity 引用，避免泄漏。翻译历史持久化到 SharedPreferences（仅 Android）并可在 `TranslationHistoryPage` 浏览。

翻译功能支持多目标语言、源语言检测（已是目标语言则原样返回）、app 上下文附带。文本来源优先使用 on-device OCR（ncnn + PPOCRv5，支持 17+ 语言和竖排文字，离线工作），OCR 无法识别时降级到无障碍快照节点。批量翻译流式输出，结果通过 `TranslationOverlayManager` 在屏幕原位渲染覆盖层（触摸穿透），支持 light/dark/stroke 样式和不透明度。OCR 返回的每个块除 `text/bounds/orientation/prob` 外还携带 `boxW/boxH/fontSize/angle`：`fontSize` 是 native 在 JNI 层把 PPOCRv5 检测阶段的 `enlarge_ratio` (1.95) 反算还原后的字形高度（原始 screenshot px），Kotlin `NcnnOcrRecognizer.parseJson` 透传，Dart `FloatingChatSessionController.normalizeOcrBlock` 在 `_extractOcrBlocks` 中写入 block map 一起回传。覆盖层 `applyStyle` 把 `fontSize` 由 px 换算成 sp（用 `scaledDensity`）并在 8~32sp 范围内 clamp，缺失时回退默认 13sp，使译文字号与原图保持一致；横排时 `maxLines` 由 `boxH/spPx` 估算并加 2 行容差再 clamp 到 1~9，`ellipsize=END` 兜底，避免过长译文溢出到相邻文本框。`updateTranslationOverlay` 现在按调用 payload 中的 `style/opacity/layoutMode` 取值，缺省才回退 `configure` 缓存，因此运行时切换设置无需重启悬浮窗即可立即生效。用户滚动目标 app 时，无障碍 `TYPE_VIEW_SCROLLED` 事件实时驱动覆盖层跟随（含横向滚动和 fling 节流，app 自身面板滚动会被包名过滤不污染覆盖层，快速 fling 上 16ms 节流会累积 delta 避免丢帧）。覆盖层被滚出屏时改为 `INVISIBLE` 软隐藏并保留 entry，回滚时 `VISIBLE` 即时复位（而非销毁后等下次 OCR 再铺），上限 ~60 个隐藏条目后超时才真销毁。拖动/缩放悬浮面板与气泡的所有 `WindowManager.updateViewLayout` 调用都包有 try/catch，视图被异步移除（锁屏、hide、权限丢失）时不会抛 `IllegalStateException` 让悬浮窗进程崩溃。缩放尽头由屏幕边界与 minW/minH 双向 clamp，面板逼近右侧时仍可继续向 minW 缩窄，但不会越过屏幕或被 minW 抬高到 `minW == effectiveMaxW` 的死锁态。滚动停止 500ms 后走增量翻译：`onTranslationScrollSettled` 有 `_settleInFlight` 互斥避免并发竞态、新块数受 `_maxOverlayBlocks` 余量限制，同样走 `hideForScreenshot` 截 OCR 防自反馈。翻译历史缓存到 SharedPreferences，最近 20 条可在面板查看。会话语义：`stopTranslation` 停止并清覆盖层但**保留** cache 供 `resumeTranslation` 继续；`clearTranslation` 彻底终止并清 cache；锁屏走 `screenOff` 通道触发 `stopTranslation`；返回 LynAI 主 app（lifecycle resumed 或点"打开"）触发 clear。
