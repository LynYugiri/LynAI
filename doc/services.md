...
`BackupService` 负责 ZIP 备份导出、读取、预览和导入。当前 schema version 为 `2`，备份类型为 `lynai.backup`。
...
| `notes` | 选中笔记、关联文件夹、storage_v2 分页、分页正文、修订和 AI 修改建议。 |
| `schedules` | 选中日程。 |
| `todoLists` | 选中待办清单。 |
...
schema v2 仍兼容旧备份。旧备份没有 `notes/pages.json` 时，导入会按 legacy 单正文笔记处理；新版 storage_v2 备份会完整保留多分页结构。
