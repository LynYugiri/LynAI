...
7. Agent 模式提供 `read_agent_memory` 和 `update_agent_memory`，用于主 Agent、Subagent 和 Lua 共享当前对话的持久化工作记忆。
8. Agent 模式提供 `run_subagent`，用于把手机自动化、屏幕读取和 OCR/识图等高噪声子任务隔离到独立上下文，主对话只接收最终结构化结果。
