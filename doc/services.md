...
| Anthropic | system 消息提升到顶层 `system`（分段内容收敛为纯文本），其余消息写入 `messages`，内容转 Anthropic block；`thinking` 由输入框的思考开关直接驱动（Anthropic 必须显式声明才思考），开启时不发 `temperature`（Anthropic 要求扩展思考下该值必须为 1）。强度分两条路径：目录给出 effort 取值的模型走原生 `thinking: {type: adaptive}` + `output_config: {effort: low/medium/high/xhigh/max}`（`minimal` 归一到 `low`，不选档位只发 adaptive）；目录只给 `budget_tokens` 的模型走预算换算（优先级 `thinkingBudgetTokens` > 档位换算 > 默认 1024）。完整 `thinking` 对象或 `thinkingBudgetTokens` 是逃生舱，显式配置时始终走预算路径。 |
...
`output_config.effort` 是 Anthropic 的原生强度参数（在 Claude Opus 4.5 上属于 beta，需要 `effort-2025-11-24`，默认 `high`；4.6+ 上由模型自适应决定思考量）。客户端按模型目录的 `reasoning_options` 判断走原生 effort 还是预算换算：目录列出 effort 取值说明该模型支持原生强度，只列 `budget_tokens` 则用 `low`/`medium`/`high` 通用档位换算预算（`reasoningEffortBudgets`，结果夹在 `max_tokens-1` 内）。托管 relay 上档位以 `reasoning.effort` 下发，由后端按上游格式换算（当前 Anthropic 上游仍走预算路径）。

OpenAI 兼容请求会显式发送 thinking 开关。部分已配置后端依赖 disabled 标记，不要随意删除。当模型目录明确说该模型不支持推理、而且用户没有显式打开该能力时，客户端不再发送 `thinking` 与强度字段（给非推理模型发这些参数只会被忽略或被部分服务端拒绝）；`extraParams` 里已有的 `thinking`/`reasoning`/`reasoning_effort` 优先于客户端计算值。思考强度为 `none` 表示关闭思考。托管 canonical 的 `reasoning.effort` 只在 `/relay/config` 顶层广告 `capabilities.reasoningEffort` 时下发：旧后端对未知字段返回 400（严格 JSON 解码），该能力位由 `ModelConfigProvider` 写进托管配置的 `extraParams.relayReasoningEffort`。Anthropic 同样复用这个开关：它的 `thinking` 字段会让服务端按扩展思考校验（要求所选模型支持、且 temperature 为 1），因此由开关直接生成标准结构并在开启时省略 `temperature`，预设里显式写 `thinking: false` 可强制关闭。注意 Anthropic 还要求 `budget_tokens >= 1024` 且小于 `max_tokens`：`maxTokens` 设到 1024 及以下时预算会被夹到 1023，服务端会直接拒绝（这是已知边界，要用扩展思考就把 maxTokens 设到 1024 以上）。
...
这些工具复用 `PluginProvider` 的串行文件队列与恢复点机制，和 Plugin Studio 编辑同一份状态。`create_plugin` 不会启用插件或授权任何运行时权限，新插件保持草稿态，待用户审查。`buildApiMessages` 在 Agent 开启且绑定工作区时注入插件摘要提示词；插件工坊经 `openPluginAiConversation` 新建工作区并复用同一套对话与工具链路。Agent 的插件开发闭环是「写文件 → `plugin_validate` 静态校验 → `plugin_run_handler` 就地试跑 → 按报错迭代」；试跑对非内置插件默认授予其声明权限，仅作用于当次运行的副本，插件启用后的真实授权仍由用户在插件管理页确认。功能页的视觉/交互效果仍只能由用户在插件工坊或功能页预览确认。
