...
删除任务会把任务及其可选清单条目写入回收站；删除清单只回收清单与条目，任务实体保留为未归类任务。Provider 先通知内存变化，再等待完整分区快照写入。
...
`TaskProvider` 与 `CalendarProvider` 在完整快照持久化成功后只通知 `CalendarPlatformProjectionCoordinator`。协调器等待两个保存队列并串行生成一份 Android 完整投影，避免两个 Provider 并发覆盖小组件和通知状态；启动、备份恢复和远端同步重载完成后也会显式同步一次。
...
当前同步覆盖对话、消息、消息附件、附件资源、`tasks`/`task_lists`/`task_list_entries`、`calendar_events`/`anniversaries`、情景演绎和回收站。资源 Blob 在引用记录之前上传或下载，并在落盘前校验 SHA-256。远端批次应用后会重载 `TaskProvider`、`CalendarProvider` 等相关 Provider，并在规划数据重载后重新同步 Android 投影，避免旧内存状态覆盖下载结果。
