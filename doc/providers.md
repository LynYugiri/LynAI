...
| `syncLynaiManagedProvider()` | 登录后从后端 `/relay/models` 同步托管 LynAI Provider，按 `api_type` 分组创建或更新配置。 |
...
模型排序先按 `category`，再按 `priority`。删除或导入替换模型后，应修复 Settings、Conversation 和 Roleplay 中的模型引用。`managed=true` 的托管 Provider 由同步流程维护，普通编辑和删除入口不会改写它们。
