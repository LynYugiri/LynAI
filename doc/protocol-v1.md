...
服务端对每条 change 的 `table` 使用固定白名单，白名单新增（或移除）任何表都属于 wire 兼容性变更：v1 期间新增过 `composer_drafts`（对话分区：对话页输入框草稿，记录形状 `{id, conversationId?, draft, updatedAt}`）。服务端对该表强制约束：`conversationId` 可省略，存在时必须是 1..256 bytes 的非空字符串；`draft` 必须是 JSON object，其中 `segments`/`attachments` 存在时必须是数组；`updatedAt` 必须是 RFC3339 时间。`composer_drafts` 在云端索引中归入 `conversations` category：`objectId` 取 payload 的 `conversationId`，未创建对话的草稿没有 `conversationId`，`objectId` 回退为记录自身 ID；因此 selective purge 与 reseed 都按该 category/object 作用域进行，对话的 object purge 会删除草稿的当前 projection 和按 payload 归属到该对话的 upsert 历史，缺少 `data` 的 delete 历史按记录 ID 归属（客户端绑定对话的草稿记录 ID 就是 `conversationId`，因此同样会被删除）。客户端按用户选择的分区上传，不会先探测后端是否支持某张表；旧后端会整批拒绝含该表的请求（HTTP 400），因此新后端必须先于新客户端部署。
...
`GET /sync/status` 和别名 `GET /sync/index/status` 返回 `lastSeq`、`generation`、`indexRevision`、`minAvailableSeq`、兼容字段 `blobCount`、`usage`、`limits` 和 additive `capabilities`。`capabilities` 是可扩展的 capability-name 到 JSON boolean 对象，当前内置 `index`、`selectivePurge`、`fullPurge`、`operationAck`，并在服务端至少配置一个搜索 provider 时广告 `webSearch: true`；模型类别能力仍以 `/relay/config` 的托管模型列表为权威，不在此重复广告。旧后端省略该 object 或未知 capability 时客户端按 false 安全降级。capabilities object 一旦存在，所有已出现的 capability 值必须是 JSON boolean，非法类型必须失败关闭；客户端分别门控索引浏览、selective purge、full purge、operation ACK 和后端网页搜索，不得用任一 capability 代替其他 capability。若后端广告 capabilities，新字段缺失、非法或分页间 generation/indexRevision 不一致时客户端必须失败关闭且不得推进 cursor。`usage` 至少包含当前 `recordCount`、`blobCount`、`blobBytes`、`blobRefCount`。新用户 `generation=1`、`minAvailableSeq=0`；`indexRevision` 是每用户单调递增的 projection revision，普通 change 至少推进到最新 seq，purge 额外推进一次，不能假定它始终等于 `lastSeq`。
...
请求体 `reasoning` 是思考控制对象：`enabled` 是布尔开关，`budgetTokens` 与 `effort` 是可选的强度表达，二者互斥（同时出现会被拒绝），`effort` 取值固定为 `none`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`（只接受小写，不做归一化），`none` 表示关闭思考（即使 `enabled: true` 也不向任何上游发送思考参数）。上游映射：OpenAI 格式写 `reasoning_effort`；Anthropic 格式把 effort 换算成 `budget_tokens`（`minimal=1024`、`low=2048`、`medium=8192`、`high=24576`、`xhigh=32768`、`max=49152`，显式 `budgetTokens` 优先，结果必须小于 `max_tokens`）；Ollama 格式映射成 `think` 的 `low`/`medium`/`high` 档位（`minimal`/`low` → `low`，`high`/`xhigh`/`max` → `high`，其余按开关）。该换算阶梯是工程取值，不代表任何厂商推荐值。客户端只在 `/relay/config` 顶层返回 `"capabilities": {"reasoningEffort": true}` 时才发送 `effort`——旧后端对未知字段 fail closed（严格 JSON 解码），所以这个 additive 能力位是兼容开关；`/relay/config` 的 `schemaVersion` 仍为 4，`capabilities` 是新增顶层键，旧客户端会忽略它。
