...
模型与接口页先展示四个模型配置分类：Chat、OCR、Speech、Image Generation，再展示本地模型、网页搜索和 MCP 服务三个相关设置入口。Chat 配置可以有多个子模型，每个子模型都可以单独设置启用状态、视觉能力、思考能力、工具能力、采样参数。子模型列表每行显示生效的上下文窗口/输出上限及其来源（手填、Endpoint、目录、Provider 或默认值），不再让"默认 256k"默默生效。思考强度不设模型级默认——弹窗里没有这一项，档位只在对话里选（见「输入区」）。Chat 配置还可以手动指定「模型目录来源」（models.dev provider），用于自建转发层或目录里没有 `api` 字段的服务。
