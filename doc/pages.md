...
输入框正文、引用 Chip 和暂存附件（图片/文件）按对话暂存（`ComposerDraftService`）：切换对话时旧对话的内容留在原对话、新对话回填自己的草稿，「新建对话」回到独立的未创建对话槽位；草稿是本地 UI 状态，不进入备份、云同步或 LAN 同步，进程结束后重新打开仍能恢复，对话被删除时一并删除。发送成功或把输入框与附件都清空会删除对应草稿；附件只记录应用私有存储中的路径，文件已被清理时该条目不再恢复。撤回把消息正文和附件一起回填输入框，因此也按当前对话暂存。
