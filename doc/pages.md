...
| 任务清单 | `features/todo_lists_page.dart` | 今日/收件箱/自定义清单/已完成视图，任务日期、提醒、排序、Markdown 导入导出和长图分享。 |
...
右上角新增菜单分别创建事件、任务和纪念日。事件支持定时/全天及跨日范围；任务支持计划、截止和完成状态；纪念日支持一次性/每年、来源年份、周年数和闰日回退。点击发生记录会打开其规范来源对象编辑器，而不是修改投影本身。
...
任务页读取 `TaskProvider`，分为今日、收件箱、自定义清单和已完成。今日按逾期、今日截止、今日计划去重聚合；收件箱是没有 `TaskListEntry` 的未完成任务。桌面宽布局使用左侧清单栏，移动端使用横向 section chips。

任务支持快速添加、标题/备注搜索、计划/截止日期与可选时间、完成状态、移动清单和多个提醒。任务提醒只能锚定计划或截止；没有具体时间时编辑器使用当天 09:00。删除清单后任务保留在收件箱；清单与清单内任务都可排序。Markdown 导入会转换成规范 `Task`/`TaskListEntry`，导出和长图分享只在页面层格式化数据。
