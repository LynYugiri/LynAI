...
- **日程表**: 月视图支持选中日期查看摘要，周视图以时间轴展示并按当天可见区间截断跨天日程，年视图按月份聚合并用可换行气泡展示日程；新增日程默认使用当前选中日期。
...
- **核心功能**:
  ...
  - **图片链路**: 多图选择或剪贴板粘贴→应用私有目录保存→OCR 或多模态图片识别→当前模型回复
  ...
  - **重试**: 支持编辑用户消息后重试, 多次重试可历史导航(< >)，带图片消息会保留图片并重新构建识别上下文
  - **编辑消息**: 点击用户消息旁的编辑图标可编辑后重发, 点击历史消息可在该位置开始新对话分支；图片附件会随新分支保留
