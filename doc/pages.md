...
首层列出内置引用源（笔记/笔记页面/待办清单/待办/知识库/知识条目/对话）与插件数据源；过滤词非空时跨源铺平命中条目，不用先选类型。进入某个源后按文件夹分层导航，**每一级都能直接引用**：列表首位是「引用整个 XXX」范围行，文件夹条目本身也带范围引用值，因此「笔记 → 文件夹」不再继续下钻就等于引用整个文件夹。选中产生 `ComposerReference`（含 `scope`），通过 `ReferenceComposerController`（`lib/widgets/reference_composer.dart`）渲染为行内不可拆分 Chip：每个 Chip 在文本模型中只占一个私用区码点，退格/删除整体移除，用户无法把光标切入 Chip 内部。发送时气泡显示 `@标题`（文件夹引用附加「（整个文件夹）」），模型侧只接收 `<lynai_ref .../>`（type/id/`scope` 与稳定限定字段，不含标题与正文）。引用随用户消息以 `composerSegments` 持久化，撤回/编辑时还原 Chip；失效引用在编辑时标记为「已失效」。插件命令若声明 `model`，选中后写入 `_pendingModelId` 覆盖本次发送模型。
