...
功能仪表盘中的“知识库”页面提供知识库、标注类别和条目管理，条目详情展示 AI 解释与来源，并保留“记忆卡片”占位。类别可配置 alias、自动标注规则、解释提示词和颜色；页面不再提供默认知识库、默认类别或“设为默认”操作。固定内置的专有名词知识库和类别显示“内置”，可编辑、启停并独立恢复模板，但不显示删除操作。条目点击仍切换右侧或紧凑详情，不再通过 Card elevation、背景色或 `ListTile.selected` 显示选中状态。
