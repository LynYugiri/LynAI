...
| 悬浮窗 | Android 悬浮权限、无障碍权限、后台气泡、悬浮聊天输入/发送/停止/新建对话、语音识别、当前页面读取、翻译（多语言、横/竖排排版、屏幕区域覆盖、滚动软隐藏后回滚可见性复位、可中途停止、可继续译、返回 LynAI 主 app 触发 clear、译文卡菜单复制/清空、屏蔽应用、翻译历史）、气泡拖动+状态指示（含垂直位置 clamp 修复）、面板拖动+缩放（手柄防越屏，位置持久化去抖）、消息长按复制、Agent Plan 运行/暂停。 |
