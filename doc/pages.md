...
`ChatPage` 协调模型选择、附件、语音、文件识别、OCR、工具调用、流式响应、失败恢复和分享。
...
历史页读取 `ConversationProvider.conversations`。搜索匹配标题和消息正文，并高亮命中片段。历史按当前角色和其他角色分组，用户可从历史页切换当前角色。
