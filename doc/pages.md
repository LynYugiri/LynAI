...
| 悬浮窗 | `floating_assistant_settings_page.dart` | Android 系统悬浮聊天、模型按需读取当前页面、语音输入、屏幕翻译（多目标语言、覆盖样式、屏蔽应用包名、专用翻译模型）、翻译历史入口、Agent Plan 显示、悬浮窗位置重置。 |
...
| 悬浮窗 | Android 悬浮权限、无障碍权限、后台气泡、悬浮聊天输入/发送/停止/新建对话、语音识别、当前页面读取、翻译（多语言、横/竖排排版、屏幕区域覆盖、译文字号随原图 OCR 字号自适应、运行时切换样式/不透明度/布局立即生效、滚动软隐藏后回滚可见性复位、可中途停止、可继续译、返回 LynAI 主 app 触发 clear、译文卡菜单复制/清空、屏蔽应用、翻译历史）、气泡拖动+状态指示（含垂直位置 clamp 修复）、面板拖动+缩放（手柄防越屏、视图被异步移除时安全吞错避免崩溃、位置持久化去抖）、消息长按复制、Agent Plan 运行/暂停。 |
