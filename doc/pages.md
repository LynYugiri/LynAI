...
| 长按对话 Tab | 长按 500ms 打开三方向快捷盘（默认 ← 日程表、↑ 笔记、→ 待办），滑向方向松手打开对应功能页。 |
| 长按修改模式 | 快捷盘打开后继续按住 3s 进入修改模式，滑向某个方向松手可选择该方向的新动作。 |
...
| 步骤 | 行为 |
|------|------|
| 你的名字 | 第一步可选填写名字，用于称呼用户并写入角色记忆、系统提示词和欢迎语。 |
...
| 编辑草稿 | 分组展示角色、角色记忆、Agent 默认值、知识库、牌组、任务清单、笔记与 SKILL；每组可编辑或删除。点「重新生成」时可选择填写不满意的原因（可选），AI 会优先按反馈修改。 |
...
| 思考按钮 | 一个控件同时表达开关与强度：点一下像模型选择那样展开列表，可选「关 / 默认 / 各强度档位」，写进对话设置（未创建对话时写草稿设置）。按钮文字固定是「思考」——关闭时灰色、开启（默认）时亮色，选了档位显示「思考 · 档位名」；提示气泡显示 关/默认/档位。档位来自模型目录的 effort 取值；目录只给 `budget_tokens` 的预算型模型（多数 Claude 直连模型）在能忠实换算的配置上（直连 Anthropic，或托管 relay 已广告能力位）回退到 `low/medium/high` 通用档位，副标题标注换算后的思考预算。 |
...
输入框正文、引用 Chip 和暂存附件（图片/文件）按对话暂存：切换对话时旧对话的内容留在原对话、新对话回填自己的草稿，「新建对话」回到独立的未创建对话槽位。草稿存在 `composer_drafts` 表，属于对话分区（随对话备份与同步，删除对话时随对话进回收站并在恢复时写回）；未创建对话的草稿只留在本机。发送成功或把输入框与附件都清空会删除对应草稿。附件以 storage_v2 Resource 为准：另一台设备认回内容后按 Resource 解析本机路径，文件缺失时显示占位而不是丢弃。撤回把消息正文和附件一起回填输入框，因此也按当前对话暂存。
...
引用面板由输入框里的 `@` 触发，按钮（图标 `@`）只是「在光标处插入一个 `@`」，与键盘触发共用同一套状态。触发规则完全由「光标前的文本」推导、不保存状态：`@` 与 `/` 必须位于文首、行首或空白之后（`/` 额外要求行首形式），token 内一旦出现空格或换行，面板立即关闭、那段文本按普通正文处理；邮箱、URL、`A/B`、`和/或` 因此永不触发。面板打开时 `↑`/`↓` 移动选中项、`Enter` 确认、`Esc` 关闭（`ChatComposerKeyboard.onPaletteKey` 只在面板打开时抢占这些键，输入法组合期间不抢 `Enter`）。

首层列出内置引用源（笔记/笔记页面/待办清单/待办/知识库/知识条目/对话）与插件数据源；过滤词非空时跨源铺平命中条目，不用先选类型。进入某个源后按文件夹分层导航，**每一级都能直接引用**：文件夹条目回车进入下一层，进入后列表首位是「引用整个 XXX」范围行，取的就是该文件夹自带的整体引用值，所以「笔记 → 文件夹」停在这一层即等于引用整个文件夹；没有自带范围值的层级（知识库条目源里的知识库）只能继续下钻。导航路径由 `_composerTrail` 维护（每项记录父 ID 与该层范围引用），笔记页面源先列出笔记、进入某篇后再列它的分页，面板头部返回按钮逐级回退、在第一层再返回则退回引用源列表。选中产生 `ComposerReference`（含 `scope`），通过 `ReferenceComposerController`（`lib/widgets/reference_composer.dart`）渲染为行内不可拆分 Chip：每个 Chip 在文本模型中只占一个私用区码点，退格/删除整体移除，用户无法把光标切入 Chip 内部。发送时气泡显示 `@标题`（文件夹引用附加「（整个文件夹）」），模型侧只接收 `<lynai_ref .../>`（type/id/`scope` 与稳定限定字段，不含标题与正文）。引用随用户消息以 `composerSegments` 持久化，撤回/编辑时还原 Chip；失效引用在编辑时标记为「已失效」。插件命令若声明 `model`，选中后写入 `_pendingModelId` 覆盖本次发送模型。

### 斜杠指令

输入框行首的 `/` 触发指令面板，内置指令默认中文命名、英文名作为别名：

| 指令 | 别名 | 行为 |
|------|------|------|
| `/压缩` | `compact` | 立即执行：把当前发送上下文里较早的历史压成摘要并写入对话检查点。原始消息保留，输入区上方出现「已压缩 N 条历史」标记条，可查看摘要或清除。再次 `/压缩` 只摘要新进入上下文的消息，不会出现「总结的总结」。仅 Agent 模式可用。 |
| `/总结` | `summarize` | 立即执行：用**当前模型**总结压缩后的上下文（与发送走同一套 `buildApiMessages` 与预算裁剪，只把 system 段换成总结专用提示词并去掉工具），结果只显示在输入区上方的卡片里，**不进入上下文**，可展开、复制或关闭；切换对话即清除。 |

指令**没有参数语法**：需要不同行为就加一条新指令，而不是解析参数（参数需要分隔符与转义，会与「`/` 后输入空格即退回普通文本」直接冲突）。未匹配到任何指令时面板只是空列表，继续输入按普通文本处理。指令不会作为正文留在输入框，也不会发给模型。

### 会话引用池

用户引用过的资源会在发送时自动沉淀进**会话引用池**（`Conversation.referencePool`）。池子对用户不可见、不进入上下文、不受压缩影响；模型可通过 `list_conversation_references` 查询清单（只返回身份与标题），再用 `read_note`/`read_task_list`/`read_knowledge_base`/`read_conversation` 等工具按 id 取正文。系统提示词只在池子非空时追加一句「本会话有引用池、可用该工具查询」，因此池子为空时零开销。
...
| 消息 | 按钮 | 说明 |
|------|------|------|
| 用户 | 复制 / 编辑 / 撤回 / 分支 | 「编辑」打开弹窗改完直接发送：最后一条就地重发，历史消息从该处另开对话后立即发送。「撤回」就地截断这条消息及其后全部内容，正文与附件回到输入框但不发送。 |
| 助手 | 复制 / 分享 / 分支 / 重新生成 | 「重新生成」只在最后一条回复上出现；「分支」复制上下文到这条回复为止。 |

撤回立即生效，并在 6 秒内提供 SnackBar「撤销」；撤销会一并还原被丢弃的消息尾部、以及撤回前的输入框内容和暂存附件。窗口内继续发送或切换会话后撤销入口自动失效。
...
| 撤回用户消息 | 截断该消息及其后内容，正文回输入框，6 秒内可撤销。 |
| 用户消息分支 | 新对话只复制该消息之前的上下文，该消息回填输入框等待用户自己发送。 |
| 助手消息分支 | 新对话复制到该回复为止的上下文，不自动请求模型。 |

分支对话是独立副本，不持久化来源关系；标题在源标题前加「（N）」，从分支再分支时序号加一。分支复制会保留 `composerSegments` 与 `modelContextContent`，并跳过截断点结尾的空助手占位。
...
| 日历 | `features/schedule_page.dart` | 默认进入日视图（日程时间轴）；查看月/日/年发生记录，创建和编辑事件、任务、纪念日及提醒；日视图支持捏合或 Ctrl/滚轮缩放，上滑收起左右箭头、模式切换与新建按钮并只保留日期标题；月视图点击日期从底部弹出当日事项列表，日期格用小红点标记。 |
...
| 知识库 | `features/knowledge_page.dart` | 管理知识库、类别、条目、解释和来源；支持搜索/清除/结果计数、高亮、多种排序、拖拽自定义顺序、详情内上一个/下一个条目与复制正文、Markdown 预览与 AI 重新生成解释。 |
...
卡片页左侧（宽屏）或顶部（窄屏）选择牌组，右侧列出卡片并支持搜索、新卡/到期过滤、手动新增/编辑/删除和停用。牌组条目显示待复习数量徽标、"待复习 N 张 · 共 M 张"与启用状态，牌组数超过阈值时提供牌组搜索；牌组设置可调整每日新卡与复习上限，并对非数字输入给出校验提示。卡片列表计数在有搜索或过滤时显示"已筛选 / 共"，卡片条目用徽标标出已停用、含提示、复习次数和来自知识库；新建/编辑卡片在缺正面或反面时就地报错，不静默丢弃输入。"开始复习"按当日学习计划显示数量，没有到期卡时禁用。AI 生成对话框从知识库选择条目，固定按**每个已选条目生成 1 张卡片**执行；只列出启用知识库下的启用条目与启用类别，条目行显示所属类别；生成结果预览可编辑正面、反面和提示，显示已覆盖/缺失条目与解析警告，保存时写入 `sourceEntryId` 溯源。学习页以牌组到期卡为会话队列，支持 Markdown/LaTeX 渲染、提示单独揭示（H）、翻面显示答案、复习卡四档评分（忘记/困难/良好/简单）、失败卡稍后重现、撤销和快捷键，并在顶部显示进度条与已复习/剩余数量。
...
日历页同时读取 `CalendarProvider` 和 `TaskProvider`，通过 `CalendarOccurrence` 统一显示日历事件、任务计划/截止和纪念日。页面默认进入日视图；月视图展示月历摘要，日视图提供可横向浏览日期的 24 小时时间轴，全天、跨日事件与纪念日、任务以每天列顶部的徽标展示，年视图用于快速定位月份。跨日事件按日期区间相交展示；同一任务计划和截止在同一天时显示为合并发生记录。
...
事件和纪念日编辑器提供常用提醒偏移。日期型提醒默认当天 09:00。提醒数据跨平台保存，系统通知、桌面卡片投影和通知权限请求在 Android 与鸿蒙上可用（Android 用 AlarmManager + 小组件，鸿蒙用后台代理提醒 + 服务卡片，见 [鸿蒙适配](harmonyos.md)）；其余平台只保存提醒数据，页面不应承诺系统级投递。
...
模型与接口页先展示四个模型配置分类：Chat、OCR、Speech、Image Generation，再展示本地模型、网页搜索和 MCP 服务三个相关设置入口。Chat 配置可以有多个子模型，每个子模型都可以单独设置启用状态、视觉能力、思考能力、工具能力、采样参数和思考强度默认值。子模型列表每行显示生效的上下文窗口/输出上限及其来源（手填、Endpoint、目录、Provider 或默认值），不再让"默认 256k"默默生效。思考强度不设模型级默认——弹窗里没有这一项，档位只在对话里选（见「输入区」）。Chat 配置还可以手动指定「模型目录来源」（models.dev provider），用于自建转发层或目录里没有 `api` 字段的服务。
...
| 日历页 | 默认进入日视图；月/日/年切换；上滑收起左右箭头、模式切换与新建按钮且只保留日期标题，下滑恢复；定时和全天跨日事件；任务计划/截止同日合并；一次性/年度纪念日、来源年份、周年数和 2 月 29 日；编辑/删除后回收站；提醒偏移和日期型 09:00。 |
...
功能仪表盘中的“知识库”页面提供知识库、标注类别和条目管理。知识库、类别和条目都支持自定义拖拽顺序；条目还可按最近更新、创建时间或标题排序，只有未搜索、未按类别过滤且使用自定义顺序时允许拖拽，避免把过滤结果或派生排序误写为持久顺序。搜索同时匹配标题和正文，标题命中优先并高亮关键词，输入框提供清除按钮；筛选区显示“共 N 条”或“筛选出 X / 共 N 条”，无匹配时展示专门的空状态并可直接清除筛选。条目列表显示类别颜色、正文摘要，以及已停用、解释条数和来源个数徽标。

条目编辑器支持 Markdown/LaTeX 编辑与预览切换，标题为空时就地提示而不静默忽略保存。详情页标题栏提供上一个/下一个条目切换、复制正文（`# 标题` + Markdown 正文写入剪贴板）和编辑入口，并展示正文、解释和来源的更新时间，解释/来源分区标题旁显示数量。解释可编辑标题和 Markdown 正文、删除，或使用条目标题、正文、所属类别及现有来源元数据重新生成并保存。同一条目生成期间会禁用重复请求，请求返回前若条目或来源关键输入已变化则丢弃旧结果。未分类条目或类别/知识库停用时会明确拒绝重新生成。来源即使为空也保留新增入口，可编辑标题、URL 和备注并删除；URL 只接受和打开 `http`/`https`，详情显示可简化为域名，但复制时保留完整 URL。删除知识库、类别、条目、解释和来源的确认弹窗使用错误色主按钮区分破坏性操作。
