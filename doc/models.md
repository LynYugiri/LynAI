...
`ConversationSettings` 保存发送对话所需的模型、系统提示词、OCR、文件识别、图片生成、语音、Agent 模式和上下文压缩快照。`contextCompressionEnabled` 控制该对话是否自动压缩与截断上下文，旧数据缺字段时默认为 true。`selectedSystemPromptId` 只保留来源标识，`systemPrompt` 保存选择当时的实际正文；发送历史对话时必须直接使用该正文，不能按当前全局模板重新解析。历史对话也不能反向覆盖全局设置。
...
| 新手向导 | `hasCompletedOnboarding`, `hasCompletedGuidedTour`, `onboardingInputJson`, `onboardingVersion` |
