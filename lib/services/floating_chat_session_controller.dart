...
class FloatingChatSessionController {
  ...
  Future<void> translateCurrentScreen() async {
    ...
    _status = '正在读取当前页面...';
    _onChanged();
    final screen = await DeviceControlService.instance.execute(
      'device.screen.context',
      const {},
    );
    if (screen['ok'] == false) {
      _setError('读取当前页面失败: ${_errorMessage(screen)}');
      return;
    }
    final result = screen['result'];
    ...
    try {
      final response = await _api.sendChatRequest(model, [
        {
          'role': 'system',
          'content': '你是屏幕文本翻译助手。将用户提供的当前页面文本翻译成简体中文，只输出译文，保留原始分段。',
        },
        {'role': 'user', 'content': text},
      ], thinking: false);
      _translationText = response.content.trim();
      _status = _translationText.isEmpty ? '未获得译文' : '已翻译当前页面';
    } catch (e) {
      _setError('翻译失败: $e');
      return;
    }
  ...
  void clearTranslation() {
    if (_translationText.isEmpty) return;
    _translationText = '';
    _status = '';
    _onChanged();
  }
